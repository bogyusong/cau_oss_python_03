...
class line:
    """
    클래스에 대한 설명은 class선언 바로 아래에 위치합니다.
    
    line class는 선의 길이에 대해 저장하고 있는 클래스입니다.
    변수로는 외부에서 접근 불가능한 __length가 있으며,
    해당 변수를 수정하고 접근하기 위해 
    set_length와, get_length 메소들를 제공합니다."""
    __width = 0
    __height = 0
    def __init__(self,width,height):
        """ 생성자 초기 width와 length값을 입력받습니다.
        Args:
            width (int or float): 초기 선의 가로 길이입니다.
            length (int or float): 초기 선의 세로 길이입니다.
        """
        self.__width=width
    ...
    def set_length(self, width, height):
        """ 선의 길이를 수정합니다.
        Args:
            width (int or float): 수정하고자 하는 선의 가로 길이입니다.
            length (int or float: 수정하고자 하는 선의 세로 길이입니다.
        """
        self.__width=width
        self.__height=height
    def get_length(self):
...
def area_rectangle(width, height):
    ...
    return width*height
def area_ecllipse(width, height):
    """ 길이를 입력받아 타원의 넓이를 구하는 함수입니다.
    Args:
        width (int or float): 장축 반의 길이입니다.
        height (int or float): 단축 반의 길이입니다.
    Returns:
